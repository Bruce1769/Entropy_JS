...
> **Entropy_JS repository:** This repo publishes a **source snapshot** of the R2R codebase paths relevant to **entropy / entropy–variance / entropy–JS** token routing. It **excludes** local experiment outputs, logs, and large weight checkpoints (see `docs/ENTROPY_JS_ROUTING.md`). Upstream: [thu-nics/R2R](https://github.com/thu-nics/R2R).
...
3. **Conda + FlashInfer JIT (`nvcc` / `-lcudart`)**  
   The HTTP server calls `ensure_cuda_jit_environment()` on import: it sets `CUDA_HOME` from the parent Miniconda install when the active env has no `nvcc`, prefers Conda’s `gcc`/`g++`, and—if the install is writable—creates `$CUDA_HOME/lib64/libcudart.so` → `../lib/libcudart.so` because FlashInfer links with `-L$cuda_home/lib64`.  
   If you still see link errors, install a full toolkit in the env (`conda install -n r2r -c nvidia cuda-nvcc`) or clear caches after fixing paths: `rm -rf ~/.cache/flashinfer`.

4. **GPU memory**  
   Quick + reference models use separate physical GPUs (`cuda:0` and `cuda:1` by default). If another job occupies GPU 0, use e.g. `CUDA_VISIBLE_DEVICES=2,3` so the pair stays free. **Qwen3-32B in bf16 needs about four 24GB GPUs for tensor parallel** (`--tp-size-ref 4` with 5 visible devices: 1 quick + 4 reference). For two 24GB cards, use a config like `Qwen3-0.6B+Qwen3-8B.yaml` instead.

5. **Slow reference startup**  
   First-time Hugging Face downloads or large checkpoints can exceed the default wait; set `R2R_LLM_STARTUP_TIMEOUT=1800` (seconds) if needed.
...
You can use the provided config, or download the pre-trained router from [this link](https://huggingface.co/nics-efc/R2R_router/tree/main) and set `router_path` in the config to the router's local path:
