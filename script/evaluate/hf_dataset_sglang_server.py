...
def parse_args():
    ...
    parser.add_argument('--lookahead_steps', type=int, default=3,
                      help='SLM rollout depth N for entropy_lookahead')
    parser.add_argument('--score_threshold', type=float, default=2.0,
                      help='S threshold for entropy_lookahead (route to LLM if S > this)')
    ...
    parser.add_argument('--use_http_server', action='store_true', default=False,
                      help='Use HTTP server for inference instead of local model')
    parser.add_argument('--server_url', type=str, default='http://0.0.0.0:30000',
                      help='URL of the R2R HTTP server (default: http://0.0.0.0:30000)')
...
def process_with_model(
    ...
    if args.use_http_server:
        # HTTP server mode - no local model initialization needed
        print(f"Using HTTP server at {args.server_url} for inference")
        ...
        try:
            ...
            print(f"HTTP server is healthy: {response.json()}")
        except requests.exceptions.RequestException as e:
            print(f"Warning: Could not verify HTTP server health: {e}")
            print("Proceeding anyway - make sure the server is running!")
        sampling_params = {
        ...
        if switching_strategy == 'rolling':
            ...
            et = router_config.get("entropy_threshold", args.threshold)
            if et is None:
                et = 0.45
...
def generate_with_http_server_single(
    input_ids: List[int],
    server_url: str,
    sampling_params: Dict,
    display_progress: bool = False
) -> Dict:
    """Send a single generation request to the HTTP server.
    
    Args:
        input_ids: Tokenized input IDs
        server_url: URL of the R2R HTTP server
        sampling_params: Sampling parameters (temperature, top_p, top_k, max_new_tokens)
    ...
    try:
        ...
        return {"text": "", "output_ids": [], "error": str(e)}
    except Exception as e:
        print(f"Error: {e}")
        return {"text": "", "output_ids": [], "error": str(e)}


def generate_with_http_server_batch(
    inputs_batch: List[List[int]],
    server_url: str,
    sampling_params: Dict
) -> List[Dict]:
    """Send multiple generation requests to the HTTP server sequentially.
    
    Args:
        inputs_batch: List of tokenized input IDs
        server_url: URL of the R2R HTTP server
        sampling_params: Sampling parameters
        
    Returns:
        List of response dictionaries from the server
    """
    results = []
    for input_ids in inputs_batch:
        result = generate_with_http_server_single(
            input_ids=input_ids,
    ...
    **kwargs_generation
    ...
    for i in tqdm(range(0, len(problems), batch_size), desc="Processing batches"):
        ...
        if args.use_http_server:
            ...
            # Send batch requests to HTTP server sequentially
            http_results = generate_with_http_server_batch(
                inputs_batch=inputs,
                server_url=args.server_url,
                sampling_params=http_sampling_params
            )
            
            # Extract generated texts from HTTP responses
            generated_texts = [result.get("text", "") for result in http_results]
            
            # Extract LLM percentage if available (for hybrid model statistics)
            llm_percentages = [result.get("llm_percentage", None) for result in http_results]
        ...
        for j, (generated_text, item) in enumerate(zip(generated_texts, batch)):
            ...
            if args.use_http_server:
                ...
                if llm_percentages[j] is not None:
                    result["reference_model_percentage"] = llm_percentages[j]
                    result["quick_model_percentage"] = 100.0 - llm_percentages[j]
                if "model_agreement_percentage" not in result:
                    result["model_agreement_percentage"] = 0
                if "quick_source_agreement_percentage" not in result:
                    result["quick_source_agreement_percentage"] = 0
                if "total_params_billions" not in result:
                    result["total_params_billions"] = 0
                if "avg_params_billions" not in result:
                    result["avg_params_billions"] = 0
    ...
    if args.use_http_server:
        # No shutdown needed for HTTP server mode
        pass
    elif use_hybrid:
        generator.shutdown()
    else:
