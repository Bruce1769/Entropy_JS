...
from r2r.evaluate.eval_utils import QUERY_TEMPLATE_AMC
...
from r2r.utils.model_paths import normalize_model_paths_in_config, tokenizer_local_files_only
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum new tokens (default: dataset-specific from dataset_configs.json, else 32768)',
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Skip this many problems from the start of the (filtered) list before applying --num_problems. '
        'E.g. offset=50 and num_problems=20 runs global indices 50–69 in the dataset order.',
    )
    ...
    parser.add_argument('--variance_threshold', type=float, default=None,
                      help='Variance threshold for entropy_variance strategy (overrides YAML if set)')
    parser.add_argument('--lookahead_steps', type=int, default=3,
                      help='SLM rollout depth N for entropy_lookahead (N+1 tokens scored)')
    parser.add_argument('--score_threshold', type=float, default=2.0,
                      help='Sum log(p_SLM/p_LLM) threshold for entropy_lookahead (route to LLM if S > this)')
    parser.add_argument('--window_size', type=int, default=5,
                      help='Window size N for sliding_window_entropy')
    parser.add_argument('--window_entropy_threshold', type=float, default=10.0,
                      help='Entropy-sum field for sliding_window_entropy (used as sum/window_size if mean not set)')
    parser.add_argument(
        '--entropy_mean_threshold',
        type=float,
        default=None,
        help='Sliding window: trigger if mean(H) > this; overrides sum/window_size default',
    )
    parser.add_argument('--intervention_mode', type=str, default='replace_first',
                      choices=['replace_first', 'replace_window', 'replace_full_window', '1', '2', '3'],
                      help='LLM intervention mode for sliding_window_entropy')
    parser.add_argument(
        '--truncate_on_llm_trigger',
        action='store_true',
        help='sliding_window_entropy: on LLM trigger, drop SLM tokens from argmax-in-window through end, then LLM',
    )
    parser.add_argument(
        '--js_threshold',
        type=float,
        default=None,
        help='sliding_window_entropy_js / entropy_variance_js: JS(SLM||LLM) threshold for routing',
    )
    parser.add_argument(
        '--rpc_timeout_s',
        type=float,
        default=None,
        help='RPC timeout in seconds for JS/lookahead helper calls (overrides YAML when set)',
    )
    ...
    if args.config_path:
        ...
        normalize_model_paths_in_config(model_config)
    ...
    # Default neural router checkpoint path from eval_configs/r2r_configs.json (per dataset)
    if args.dataset in R2R_CONFIGS:
        rc = R2R_CONFIGS[args.dataset]
        router = model_config.setdefault("router", {})
        if rc.get("router_path") and not router.get("router_path"):
            router["router_path"] = rc["router_path"]
            print(f"Using router_path from r2r_configs for dataset {args.dataset}: {router['router_path']}")
    ...
    if args.max_new_tokens is None:
        args.max_new_tokens = int(dataset_config.get("max_new_tokens", 32768))
        print(f"Using max_new_tokens={args.max_new_tokens} (dataset default for '{args.dataset}')")
...
def process_problems(
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        local_files_only=tokenizer_local_files_only(args.model_path),
    )
...
def process_with_model(
    ...
    if use_hybrid:
        ...
        if switching_strategy == 'rolling':
        ...
        elif switching_strategy == 'entropy_variance':
            et = router_config.get("entropy_threshold")
            if et is None:
                et = args.threshold
            if et is None:
                et = 0.45
            vt = router_config.get("variance_threshold")
            if vt is None and getattr(args, "variance_threshold", None) is not None:
                vt = args.variance_threshold
            if vt is None:
                vt = 1e-5
            strategy_kwargs.update({
        ...
        elif switching_strategy == 'entropy_variance_js':
            et = router_config.get("entropy_threshold")
            if et is None:
                et = args.threshold
            if et is None:
                et = 0.45
            jt = getattr(args, "js_threshold", None)
            if jt is None:
                jt = router_config.get("js_threshold")
            if jt is None:
                jt = 0.1
            slp = router_config.get("score_log_path")
            if not slp and getattr(args, "output_dir", None):
                slp = os.path.join(args.output_dir, "entropy_variance_js_score_log.jsonl")
            strategy_kwargs.update({
                'entropy_threshold': float(et),
                'js_threshold': float(jt),
            })
            if slp:
                strategy_kwargs["score_log_path"] = slp
            rto = getattr(args, "rpc_timeout_s", None)
            if rto is None:
                rto = router_config.get("rpc_timeout_s")
            if rto is not None:
                strategy_kwargs["rpc_timeout_s"] = float(rto)
        elif switching_strategy == 'entropy_lookahead':
            et = router_config.get("entropy_threshold")
            if et is None:
                et = args.threshold
            if et is None:
                et = 0.45
            ls = router_config.get("lookahead_steps", getattr(args, "lookahead_steps", 3))
            st = router_config.get("score_threshold", getattr(args, "score_threshold", 2.0))
            slp = router_config.get("score_log_path")
            if not slp and getattr(args, "output_dir", None):
                slp = os.path.join(args.output_dir, "entropy_lookahead_score_log.jsonl")
            strategy_kwargs.update({
                'entropy_threshold': float(et),
                'lookahead_steps': int(ls),
                'score_threshold': float(st),
            })
            if slp:
                strategy_kwargs["score_log_path"] = slp
        elif switching_strategy == 'sliding_window_entropy':
            ws = router_config.get("window_size", getattr(args, "window_size", 5))
            wet = router_config.get("entropy_sum_threshold")
            if wet is None:
                wet = getattr(args, 'window_entropy_threshold', 10.0)
            mode = router_config.get("intervention_mode", getattr(args, "intervention_mode", 'replace_first'))
            slp = router_config.get("score_log_path")
            if not slp and getattr(args, "output_dir", None):
                slp = os.path.join(args.output_dir, "sliding_window_entropy_score_log.jsonl")
            tot = router_config.get(
                "truncate_on_llm_trigger",
                getattr(args, "truncate_on_llm_trigger", False),
            )
            emt = router_config.get("entropy_mean_threshold")
            if emt is None:
                emt = getattr(args, "entropy_mean_threshold", None)
            strategy_kwargs.update({
                'window_size': int(ws),
                'entropy_sum_threshold': float(wet),
                'intervention_mode': mode,
                'truncate_on_llm_trigger': bool(tot),
            })
            if emt is not None:
                strategy_kwargs["entropy_mean_threshold"] = float(emt)
            if slp:
                strategy_kwargs["score_log_path"] = slp
        elif switching_strategy == 'sliding_window_entropy_js':
            ws = router_config.get("window_size", getattr(args, "window_size", 5))
            wet = router_config.get("entropy_sum_threshold")
            if wet is None:
                wet = getattr(args, 'window_entropy_threshold', 10.0)
            ...
            slp = router_config.get("score_log_path")
            if not slp and getattr(args, "output_dir", None):
                slp = os.path.join(args.output_dir, "sliding_window_entropy_js_score_log.jsonl")
            tot = router_config.get(
                "truncate_on_llm_trigger",
                getattr(args, "truncate_on_llm_trigger", False),
            )
...
def evaluate_problem(
    ...
    **kwargs_generation
    ...
    pbar = tqdm(range(0, len(problems), batch_size), desc="Processing batches")
    for i in pbar:
        ...
        if batch_speed is not None:
            pbar.set_postfix(speed=f"{batch_speed:.1f} tok/s")
        else:
            pbar.set_postfix(speed="n/a")
...
def preprocess_dataset(dataset, dataset_config: Dict, save_result_dir: str) -> List[Dict]:
    ...
    id_prefixes = dataset_config.get("id_prefix_filter")
    if id_prefixes:
        _idf = id_field
        dataset = dataset.filter(
            lambda x: any(str(x[_idf]).startswith(p) for p in id_prefixes)
        )
    ...
    for idx, item in enumerate(dataset):
        ...
        if dataset_config.get("sanitize_id_for_filesystem"):
            processed_item["ID"] = (
                processed_item["ID"].replace("/", "_").replace("\\", "_").replace(":", "_")
            )
        ...
        elif dataset_config.get("answer_type") == "amc":
            tpl = dataset_config.get("prompt_template")
            if tpl:
                processed_item["FormattedProblem"] = tpl.format(question=processed_item["Problem"])
            else:
                processed_item["FormattedProblem"] = QUERY_TEMPLATE_AMC.format(
                    question=processed_item["Problem"]
                )
...
def main():
    ...
    if args.dataset_config_dict.get("answer_type") == "mmlu-multiple-choice":
        ...
        hf_split = args.dataset_config_dict.get("hf_split")
        if hf_split:
            if hf_split not in dataset:
                raise ValueError(
                    f"hf_split={hf_split!r} not found in dataset. Available splits: {list(dataset.keys())}"
                )
            dataset_split = dataset[hf_split]
            print(f"Using Hugging Face split from config: {hf_split!r} ({len(dataset_split)} examples)")
        else:
    ...
    if args.problem_offset:
        if args.problem_offset < 0:
            print("Error: --problem_offset must be >= 0")
            sys.exit(1)
        if args.problem_offset >= len(problems):
            print(f"Error: --problem_offset {args.problem_offset} >= number of problems {len(problems)}")
            sys.exit(1)
        problems = problems[args.problem_offset :]
        print(f"After --problem_offset {args.problem_offset}: {len(problems)} problem(s) in window (global order)")

    # Cap dataset size before --resume so resume only skips within this subset (e.g. first 50 + resume).
    if args.debug:
        problems = problems[:1]
        print("Debug mode: processing only first problem")
    elif args.num_problems:
        problems = problems[: args.num_problems]
        print(f"Limited to next {args.num_problems} problem(s) in window (preprocessed total {len(all_problems)})")

    if args.resume and not args.split_jobs:
        n_before = len(problems)
        problems = [p for p in problems if p["ID"] not in completed_problems]
        n_done = n_before - len(problems)
        if n_done:
            print(f"Resume: skipping {n_done} already completed in this problem set, {len(problems)} remaining")
        elif args.resume:
            print(f"Resume: {len(problems)} problems in set, none skipped (no matching temp/*.txt yet)")
