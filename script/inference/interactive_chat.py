...
# FlashInfer JIT: must set CUDA_HOME / PATH to a real nvcc before torch/sglang (see README troubleshooting).
...
import yaml
from r2r.utils.config import QUICK_COLOR, REFERENCE_COLOR, RESET
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Reference model tensor parallel size. 1 = quick on cuda:0, reference on cuda:1 (typical for Qwen3-8B on 24GB×2). '
        '2 = reference sharded on cuda:0,1 with quick colocated on cuda:1 (needs both cards mostly free on cuda:1).',
    )
