...
def compute_js_divergence_topk_union(
    ...
    P is computed from full ``logits_p`` on SLM side.
    Q is represented by compact payload from LLM side:
    ``q_topk_indices``, ``q_topk_probs`` and ``q_tail_mass``.
    """
    if logits_p.dim() == 2:
    ...
    q_pairs = list(zip(q_topk_indices, q_topk_probs))
    q_map = {}
    for idx, prob in q_pairs:
        ii = int(idx)
        if 0 <= ii < vocab:
    ...
    q_core = torch.as_tensor([q_map.get(int(i), 0.0) for i in support], dtype=torch.float32, device=p.device)
