...
from r2r.utils.metrics import compute_logu, compute_entropy, compute_variance
...
class EntropyVarianceSwitching(ModelSwitchingStrategy):
    """免训练路由策略：结合熵和概率方差做即时切换。
    熵高 + 方差小 = 概率在大量 token 上均匀分散，模型最不自信，应切到大模型。
    熵高 + 方差大 = 虽不确定但概率集中在少数候选，不确定性较低，可留在小模型。
    """
    ...
    def __init__(
        ...
        self.variance_threshold = float(variance_threshold) if variance_threshold is not None else 1e-5

        print(f"EntropyVarianceSwitching: entropy_threshold={self.entropy_threshold}, "
              f"variance_threshold={self.variance_threshold}")

    def route(self, outputs) -> torch.Tensor:
        """
        对 batch 中每个样本，同时计算熵和概率方差。
        仅当 entropy > entropy_threshold 且 variance < variance_threshold 时路由到大模型。

        Returns:
            torch.Tensor: shape [batch_size]，0=quick, 1=reference
        """
        batch_size = outputs.logits.shape[0]
        next_token_logits = outputs.logits[:, -1, :]  # [batch_size, vocab_size]

        entropy = compute_entropy(next_token_logits)    # [batch_size]
        variance = compute_variance(next_token_logits)  # [batch_size]

        if isinstance(entropy, float):
            entropy = torch.tensor([entropy], device=next_token_logits.device)
        if isinstance(variance, float):
            variance = torch.tensor([variance], device=next_token_logits.device)

        high_entropy = entropy > self.entropy_threshold
        low_variance = variance < self.variance_threshold
        model_choices = (high_entropy & low_variance).to(torch.int)
...
class EntropyVarianceJsSwitching(ModelSwitchingStrategy):
    """熵门控（仅 H > entropy_threshold）通过后，再算当前步 JS(SLM || LLM)；仅当 JS > js_threshold 时切 LLM。

    一次 LLM forward 得到下一 token 的 logits；若路由到 LLM，复用该 logits 采样首 token，
    避免 LLM 为同一分布再次 prefill（与 sliding_window_entropy_js 的复用路径一致）。

    配置里的 ``variance_threshold`` 若存在会被忽略（仅为与旧 YAML 兼容）。

    需要 SLM tp_size=1、LLM tp_size=1。无 RPC 时 ``route()`` 退化为仅按熵阈值路由（不做 JS）。
    """
    ...
    def route(self, outputs: ModelOutputs) -> torch.Tensor:
        """无 RPC 时：仅 H > entropy_threshold 则切大模型（与 SLM 侧熵门控一致，不含 JS）。"""
        batch_size = outputs.logits.shape[0]
...
class EntropyLookaheadSwitching(ModelSwitchingStrategy):
    """When SLM entropy exceeds a threshold, draft N more SLM tokens and compare to LLM.

    Score S = sum_k (log p_SLM(t_k | c_k) - log p_LLM(t_k | c_k)) on the drafted continuation.
    If S > score_threshold, route this step to the reference (LLM) model; otherwise keep SLM.

    Full lookahead + LLM log-probs are implemented in SLMServer (requires SLM tp=1, LLM tp=1).
    If TP > 1, falls back to plain entropy routing for that step.
    """
    ...
    def route(self, outputs: ModelOutputs) -> torch.Tensor:
        """Entropy-only fallback if something calls the factory API without SLMServer hooks."""
        batch_size = outputs.logits.shape[0]
        next_token_logits = outputs.logits[:, -1, :]
        model_choices = torch.zeros(batch_size, dtype=torch.int, device=next_token_logits.device)
        for i in range(batch_size):
...
class SlidingWindowEntropySwitching(ModelSwitchingStrategy):
    """Sliding-window per-step SLM entropies (no draft lookahead).

    Each decode step appends H(logits) to a per-request deque of length N.
    When the deque is full, **mean** entropy is compared to
    ``entropy_mean_threshold``. If that key is omitted in config, it defaults to
    ``entropy_sum_threshold / N`` so existing ``entropy_sum_threshold`` YAMLs
    keep a comparable scale (sum threshold was for the old sum-based rule).

    Between checks the deque is a **rolling** window. LLM decode steps do not
    push entropies; the deque is cleared on LLM trigger.

    ``truncate_on_llm_trigger``: if True (and ``intervention_mode`` is not
    ``replace_full_window``), on trigger ``slm_server`` truncates the SLM suffix from
    the **leftmost position of maximum entropy within the window** through the end
    (ties: smallest index).

    ``intervention_mode``:

    - ``replace_first``: after trigger, LLM decodes one step then back to SLM.
    - ``replace_window``: truncate as above if enabled; then LLM decodes **N** steps
      (first trigger + ``N-1`` follow-ups), then back to SLM.
    - ``replace_full_window``: on trigger, drop the **entire** last-**N** SLM tokens
      (the full window), then LLM decodes **N** steps like ``replace_window``; entropy
      deque clears and refills from SLM-only steps afterward. Ignores
      ``truncate_on_llm_trigger`` for anchor choice (always full-window drop).

    Ignored keys such as ``window_align_mode`` may appear in old configs and are
    absorbed via kwargs.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        _ = kwargs  # e.g. deprecated window_align_mode in old YAML
    ...
    def route(self, outputs: ModelOutputs) -> torch.Tensor:
        """Fallback without per-request deque state: stay on quick (no routing)."""
        batch_size = outputs.logits.shape[0]
...
class SlidingWindowEntropyJsSwitching(SlidingWindowEntropySwitching):
    """Sliding-window **mean entropy** gate, then left-to-right JS(SLM || LLM) for the anchor.

    After the window is full, if **mean entropy** exceeds ``entropy_mean_threshold``,
    the SLM requests LLM next-token logits **in order** from the oldest to the newest
    window position. After each LLM forward, JS is computed; **as soon as**
    ``JS > js_threshold``, that index becomes the truncate anchor and **no further LLM
    forwards** run for this decode step. If no position exceeds ``js_threshold`` after
    all ``window_size`` forwards, the anchor is **argmax JS** over the full window.

    Requires SLM tp_size=1 and LLM tp_size=1 (same RPC path as entropy lookahead).
    """
...
def create_switching_strategy(strategy_name: str, **kwargs) -> ModelSwitchingStrategy:
    ...
    strategies = {
        ...
        'entropy_variance': EntropyVarianceSwitching,
