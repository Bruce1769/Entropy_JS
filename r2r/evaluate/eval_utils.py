...
# AMC (and similar) use five choices A–E; problem text often already lists options.
QUERY_TEMPLATE_AMC = """
Solve the following multiple-choice problem. The statement below includes all choices (A–E).
Think step by step. The last line of your response must be exactly: Answer: $LETTER$
where LETTER is A, B, C, D, or E.

{question}
""".strip()

ANSWER_PATTERN_AMC = r"(?i)Answer[ \t]*:[ \t]*\$?([A-E])\$?"
...
def check_answer_correctness(predicted: str, actual: str, answer_type: str) -> bool:
    ...
    elif answer_type == "amc":
        return predicted.upper().strip() == str(actual).upper().strip()
...
_BOXED_AMC_INNER = re.compile(
    r"\\boxed\{((?:[^{}]|\{[^{}]*\})*)\}",
    re.DOTALL,
)


def extract_amc_answer(text: str) -> Tuple[str, bool]:
    """Extract AMC-style letter (A–E).

    Many models follow math benchmarks and write ``\\boxed{E}`` or prose like
    "the answer is E" instead of a strict final line ``Answer: E``; handle both.
    """
    if not (text and text.strip()):
        return "", False

    m = re.search(ANSWER_PATTERN_AMC, text)
    if m:
        return m.group(1).upper(), True

    for boxed in reversed(list(_BOXED_AMC_INNER.finditer(text))):
        inner = boxed.group(1).strip()
        inner = re.sub(r"\\[a-zA-Z]+\{([A-Ea-e])\}", r"\1", inner)
        inner = inner.strip()
        if re.fullmatch(r"[A-Ea-e]", inner):
            return inner.upper(), True
        lm = re.search(r"\b([A-Ea-e])\b", inner)
        if lm and len(inner) <= 48:
            return lm.group(1).upper(), True

    tail = text[-1200:] if len(text) > 1200 else text
    prose_patterns = [
        r"(?i)\*\*option\s+([A-Ea-e])\*\*",
        r"(?i)\boption\s*\(?\s*([A-Ea-e])\s*\)?\b",
        r"(?i)\b(?:the\s+)?correct\s+answer\s+is\s+([A-Ea-e])\b",
        r"(?i)\b(?:therefore|thus),?\s+the\s+answer\s+is\s+([A-Ea-e])\b",
        r"(?i)\bso\s+the\s+answer\s+is\s+([A-Ea-e])\b",
        r"(?i)\banswer\s+is\s+([A-Ea-e])\b",
        r"(?i)\bchoice\s+([A-Ea-e])\b",
    ]
