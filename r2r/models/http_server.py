...
# FlashInfer / Triton JIT: conda envs often lack nvcc; set CUDA_HOME before torch/sglang import.
...
async def lifespan(app: FastAPI):
    ...
    if server_args:
        ...
        if switching_strategy == "neural":
            ...
            if "variance_threshold" in router_config:
                strategy_kwargs["variance_threshold"] = router_config["variance_threshold"]
                print(f"Using variance threshold from config: {router_config['variance_threshold']}")
            if switching_strategy == "entropy_lookahead":
                if "lookahead_steps" in router_config:
                    strategy_kwargs["lookahead_steps"] = router_config["lookahead_steps"]
                if "score_threshold" in router_config:
                    strategy_kwargs["score_threshold"] = router_config["score_threshold"]
            if switching_strategy == "sliding_window_entropy":
                if "window_size" in router_config:
                    strategy_kwargs["window_size"] = router_config["window_size"]
                if "entropy_sum_threshold" in router_config:
                    strategy_kwargs["entropy_sum_threshold"] = router_config["entropy_sum_threshold"]
                if "entropy_mean_threshold" in router_config:
                    strategy_kwargs["entropy_mean_threshold"] = router_config[
                        "entropy_mean_threshold"
                    ]
                if "intervention_mode" in router_config:
                    strategy_kwargs["intervention_mode"] = router_config["intervention_mode"]
                if "truncate_on_llm_trigger" in router_config:
                    strategy_kwargs["truncate_on_llm_trigger"] = router_config[
                        "truncate_on_llm_trigger"
                    ]
                if "score_log_path" in router_config:
                    strategy_kwargs["score_log_path"] = router_config["score_log_path"]
            if switching_strategy == "sliding_window_entropy_js":
                if "window_size" in router_config:
                    strategy_kwargs["window_size"] = router_config["window_size"]
                if "entropy_sum_threshold" in router_config:
                    strategy_kwargs["entropy_sum_threshold"] = router_config["entropy_sum_threshold"]
                ...
                if "score_log_path" in router_config:
                    strategy_kwargs["score_log_path"] = router_config["score_log_path"]
            if switching_strategy == "entropy_variance_js":
                if "js_threshold" in router_config:
                    strategy_kwargs["js_threshold"] = router_config["js_threshold"]
                if "rpc_timeout_s" in router_config:
                    strategy_kwargs["rpc_timeout_s"] = router_config["rpc_timeout_s"]
                if "score_log_path" in router_config:
                    strategy_kwargs["score_log_path"] = router_config["score_log_path"]
