"""
R2R Router 模块：负责 token 级路由决策，判断每个 token 应由小模型还是大模型生成。

主要功能：
1. 模型注册与工厂：支持多种 Router 架构的注册、创建、保存、加载
2. Router 架构：多种基于 logits/hidden_states 的二分类/多分类器
3. 阈值优化：为 Router 输出寻找最优分类阈值
"""
...
# ==================== 模型注册、保存与加载 ====================
# 全局模型注册表：类名 -> 类，用于工厂函数 create_classifier 按名称创建实例
MODEL_REGISTRY = {}


def register_model(cls=None, name=None):
    """
    将模型类注册到全局 MODEL_REGISTRY。
    可作为装饰器使用：@register_model 或 @register_model(name="xxx")
...
def capture_init_args(cls):
    """
    装饰器：自动捕获并保存模型类的 __init__ 参数到 self._init_args，
    便于 save_model 时完整保存配置，load_model 时正确恢复。
...
def create_classifier(model_arch: str, **kwargs) -> nn.Module:
    """
    工厂函数：根据架构名称从 MODEL_REGISTRY 创建分类器实例。
...
def save_model(
    ...
    **kwargs,
    ...
    """
    保存训练好的 Router 模型及其配置（init_args、threshold、state_dict 等）。
    会移除 token_embeddings.weight（与 backbone 共享，推理时从 backbone 获取）。
...
def load_model(model_path: str, device: str = "cuda", override_init_args: dict = None, **kwargs) -> tuple[nn.Module, dict]:
    """
    加载模型：支持本地 .pt 或 HuggingFace 仓库路径。
    override_init_args 可覆盖部分 init_args（如 pretrained_model_name）。
...
class ClassifierBlock(nn.Module):
    """
    单个 Transformer 风格的 MLP 块：LayerNorm + MLP（带扩展因子）+ 残差连接。
    作为 ClassifierBackbone 的基本构建单元。
...
class ClassifierBackbone(nn.Module):
    """
    所有 Router 分类器共用的 backbone：多层 ClassifierBlock + 可选位置编码。
    输入投影 -> 位置编码（可选）-> 若干 MLP 块 -> 输出层（二分类 logits）。
...
class LogitsClassifier(nn.Module):
    """
    仅以 logits 为输入的 Router：小模型 top-k logits -> 二分类（是否需大模型）。
    """
...
class HiddenStatesClassifier(nn.Module):
    """
    仅以 hidden_states 为输入的 Router：小模型最后一层 hidden -> 二分类。
    """
...
class HiddenStatesClassifierWithLMHead(nn.Module):
    """
    使用预训练 LM head 处理 hidden_states：hidden -> LM head -> top-k logits -> backbone -> 二分类。
    利用预训练 LM head 将 hidden 转为“伪 logits”，再送入 backbone。
...
class HiddenStatesTokenLMHeadClassifier(nn.Module):
    """
    输入为 hidden_states + 采样得到的 token：拼接后送入 backbone 做二分类。
    使用预训练 token embedding 层对 token 编码。
...
class HiddenStatesTokenLMHeadLogitsClassifier(nn.Module):
    """
    输入为 hidden_states + token + logits 的 Router：三者拼接后送入 backbone 做二分类。
    信息最丰富，常用于高精度路由。
...
class HiddenStatesLogitsClassifier(nn.Module):
    """
    输入为 hidden_states + logits 的 Router：二者拼接后送入 backbone 做二分类。
    不使用 token embedding，参数量较 HiddenStatesTokenLMHeadLogitsClassifier 更小。
    This model accepts hidden states, sampled token, and logits as input.
    Uses the ClassifierBackbone for processing.
    """
...
class HiddenStatesTokenClassifierWithLMHead(nn.Module):
    """
    输入为 hidden_states + token 的 Router（带 LM head 变体）。当前未实现（raise NotImplementedError）。
    This model accepts hidden states, sampled token as input.
    Uses the ClassifierBackbone for processing.
    """
...
class MultiLogitsClassifier(nn.Module):
    """
    带神经窗口的 Router：使用连续多个 token 的 logits 拼接作为输入（neural_window_size）。
    可捕捉局部上下文中的路由模式。
    Uses the ClassifierBackbone for processing.
    """
...
class MultiHiddenStatesClassifier(nn.Module):
    """
    带神经窗口的 Router：使用连续多个 token 的 hidden_states 拼接作为输入。
    Uses the ClassifierBackbone for processing.
    """
...
# ==================== 阈值优化 ====================
# Router 输出为 [0,1] 概率，需选定阈值决定何时切到大模型。
# ThresholdOptimizer 在验证集上搜索满足 recall/fp_rate 约束的最优阈值。


class ThresholdOptimizer:
    """
    阈值优化器：在验证集上搜索最优分类阈值。
    支持普通阈值和滞后（hysteresis）阈值两种模式。
    """
    ...
    def calculate_metrics(self, threshold):
        """对给定阈值计算 recall、precision、fp_rate 等指标。"""
    ...
    def calculate_metrics_hysteresis(self, threshold, window_size=3):
        """
        滞后阈值模式：从 quick 切到 reference 需单次超阈；从 reference 切回 quick 需窗口内平均低于阈值。
        可减少频繁切换带来的抖动。
        """
    ...
    def _find_best_compromise_threshold(self, min_recall, mode="default"):
        """当无满足约束的阈值时，在满足 min_recall 的候选中选 fp_rate 最小的作为折中。"""
    ...
    def find_best_threshold(self, max_fp_rate=0.1, min_recall=0.9):
        """在满足 min_recall 和 max_fp_rate 的阈值中，选择 fp_rate 最小的。若无则用折中策略。"""
        thresholds = np.linspace(0.01, 0.99, 100)
        valid_thresholds = []
...
class MultiClassClassifierBackbone(nn.Module):
    """
    多分类 backbone：与 ClassifierBackbone 结构类似，但输出为 num_classes 维（可接 Softmax）。
    用于 MATCH/NEUTRAL/DIVERGENT 等三分类场景。
...
# ---------- 多分类 Router（MATCH/NEUTRAL/DIVERGENT 等） ----------

@register_model
@capture_init_args
class MultiClassLogitsClassifier(nn.Module):
    """
    多分类 Router：输入为 logits，输出为 num_classes 维概率（如 MATCH/NEUTRAL/DIVERGENT）。
    """
    def __init__(
        self,
        logits_size,
...
class MultiClassHiddenStatesClassifier(nn.Module):
    """
    多分类 Router：输入为 hidden_states，输出为 num_classes 维概率。
    """
    def __init__(
        self,
        hidden_states_size,
...
class MultiClassHiddenStatesClassifierWithLMHead(nn.Module):
    """
    多分类 Router：hidden_states -> 预训练 LM head -> backbone -> num_classes 维概率。
    This model only accepts hidden states as input and does not use logits.
